...
RWLock mutex_equipo1; //RWLook para el tablero del equipo 1
RWLock mutex_equipo2; //RWLook para el tablero del equipo 2
	...
	while (true) {
        ...
        //agrego al tablero una b para diferenciar las artes de barcos de los barcos terminados.
				if(soy_equipo_1) {
					...
					mutex_equipo1.wunlock();
				} else {
					mutex_equipo2.wlock();
					(*tablero_jugador)[ficha.fila][ficha.columna] = 'b';
					mutex_equipo2.wunlock();
				}
